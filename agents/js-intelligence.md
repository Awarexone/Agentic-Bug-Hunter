...
description: JavaScript intelligence agent. Analyzes JS bundles, source maps, and API routes surfaced by recon for hidden endpoints, internal URLs, feature flags, debug routes, leaked configuration, and authentication flows. Writes recon/<target>/js-intelligence.md and routes every finding into the lead board. Use after /recon, before hypothesis-engine.
tools:
  read: true
  bash: true
  glob: true
...
5. Route every real finding into the lead board so it survives beyond this file:
   ```bash
   ...
   ```
   Pick the skill that matches the finding type (`hunt-source-leak` for maps/config leaks, `hunt-api-misconfig` or `hunt-idor` for hidden endpoints depending on shape, `hunt-oauth` for auth-flow findings, `hunt-ssrf` for internal-URL fetch patterns).
