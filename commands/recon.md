...
- **`subdomains/all.txt` is empty for IP/CIDR target** — expected; the script skips subdomain enum and writes the IP directly.
- **JSON inventory shows `total_discovered: 0`** — `recon_engine.sh` failed mid-way; re-run or check the script's echo summary for which phase ran. Use `--no-dns` flag if DNS is blocked.
- **`TypeError: unsupported operand type(s) for |: 'type' and 'NoneType'`** — your `python3` is < 3.10. Run with `python3.13 tools/recon_to_json.py ...`. macOS: install via `brew install python@3.13`.
